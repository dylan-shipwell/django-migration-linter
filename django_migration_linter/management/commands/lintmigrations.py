...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "if specified, read_migrations_list migration files will be"
                " indexed by their file system prefix, relative to"
                " 'settings.BASE_DIR', which should match the loaded django"
                " appconfig's path. on IndexError falls-through to normal"
                " folder-named must be app_label based app_config mapping."
            ),
        )
